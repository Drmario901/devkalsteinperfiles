...
if (isset($_POST['remitente_id'])) {
    ...
    if ($userResult && $userResult->num_rows > 0) {
        ...
        try {
            ...
            $mail->isSMTP();
            $mail->Host = 'testing.kalstein.digital';  // Host de conexión SMTP
            $mail->SMTPAuth = true;
            $mail->Username = 'no-reply@testing.kalstein.digital';                 // Usuario SMTP
            $mail->Password = '-GAo^)d)F}j8';                           // Password SMTP
            $mail->SMTPSecure = 'tls';                            // Activar seguridad TLS
            $mail->Port = 587;                                    // Puerto SMTP
            
            $mail->setFrom('no-reply@testing.kalstein.digital');    // Mail del remitente
