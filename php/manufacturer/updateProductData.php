<?php
    ...
    if ($val){
        ...
        if ($datos['status'] == 'correcto'){
            ...
            $sqlImage = $conexion->query("SELECT product_image FROM wp_k_products WHERE product_aid = $t")->fetch_assoc()['product_image'];
            ...
            $sql4 = "INSERT INTO wp_posts (post_author, post_date, post_date_gmt, post_title, post_mime_type, post_status, post_type, guid)  
                                    VALUES (1,NOW(),NOW(),'$sqlImage','image/jpeg','inherit','attachment', '$sqlImage')";
            ...
            $sql7 = "INSERT INTO wp_postmeta (post_id, meta_key, meta_value)
                    VALUES ($image_id, '_wp_attached_file', '$sqlImage')";
