...
jQuery(document).ready(function ($) {
  ...
  function addLinkClickListeners() {
    let links = ["inbox-compose", "inbox-inbox", "inbox-sent", "inbox-view"];
  ...
  function incomingMessagesBlob() {
    $.ajax({
      url:
        "https://dev.kalstein.plus/plataforma/wp-content/plugins/kalsteinPerfiles" +
        "/php/getUnseenMessages.php",
    })
      .done(function (response) {
        let count = JSON.parse(response).unseen;

        if (
          JSON.parse(response).total > count_messages &&
          !first_message_fill
        ) {
