...
let domain = 'https://dev.kalstein.plus/index.php/rentalsale/';
...
function validateProductData(name, brand, model, condition, description, category, fileInput, stock, status, weight, length, width, height, weight_pa, length_pa, width_pa, height_pa, pa_type, price, currency, dont_image = false){
    ...
    if (name === '') {
      ...
      scroll('#productBrand');
        ...
        err_msg = alertsTranslations.productoCondicionVacio;
        scroll('#usedEquipmentCondition');
    }
    else if (description === '') {
        err_msg = alertsTranslations.descripcionVacia;
    ...
    // negative verification
        ...
        err_msg = alertsTranslations.alturaEmpaquetadoMenor;
        scroll('#heProductPa');
    }
...
jQuery(document).ready(function($) {
    ...
    $(document).on('click', '#btnSendData', function(e) {
        ...
        var condition = $('#usedEquipmentCondition').val();
        ...
        if (validateProductData(name, brand, model, condition, description, category, fileInput, stock, status,
        weight, length, width, height,
        weight_pa, length_pa, width_pa, height_pa, pa_type,
        price, currency)){

            savedDataUpload(name, brand, model, condition, description, category, fileInput, stock, status,
                weight, length, width, height,
                weight_pa, length_pa, width_pa, height_pa, pa_type,
                price, currency, manual, catalog,
                longDescription, longDescriptionCSV);
        }
    });

    function savedDataUpload(name, brand, condition, model, description, category, fileInput, stock, status,
        weight, length, width, height,
        weight_pa, length_pa, width_pa, height_pa, pa_type,
        price, currency,manual, catalog,
        longDescription, longDescriptionCSV) {
       ...
       $('#btnSendData').attr('disabled', '');
        ...
        $.ajax({
            ...
            url: plugin_dir + 'php/rentalsale/uploadProductDataUsed.php',
            type: 'POST',
            data: formData,
            dataType: 'text',
            processData: false,
                    ...
                    .done(function(response){
                        ...
                        if(response == 'done'){
                            ...
                            if(document.querySelector("#product-table-body").children.length == 0){
                                let message = alertsTranslations.noTienesStock; // Este mensaje puede ser dinámico.
                                // Insertando el mensaje dinámico en el HTML
                                $("#missing-data-msg").html("<!--center><span class='material-symbols-rounded icon'>sentiment_dissatisfied</span></center--><center><p style='color: #000;'>" + message + "</p></center>");
                            }
