...
jQuery(document).ready(function($){
    ...
    $(document).on('click', '#btnContinueLogIn2', function(){
        ...
        if ($.browser.webkit){
            browser = 'Google Chrome v'+$.browser.version
        }

        if ($.browser.safari){
            browser = 'Safari v'+$.browser.version
        }

        if ($.browser.mozilla){
            browser = 'Mozilla Firefox v'+$.browser.version
        }
        ...
        if ($.browser.msie){
            browser = 'Internet Explorer v'+$.browser.version
        }
