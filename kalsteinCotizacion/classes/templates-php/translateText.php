...
function translateText($translationsFile = 'translations.php') {
    ...
    foreach ($translations[$lang] as $key => $value) {
        ...
        // Specifically target elements with data-placeholder attribute for the placeholder text
        $html .= "var placeholderElements = document.querySelectorAll('[data-placeholderp=\"$key\"]');";
        $html .= "placeholderElements.forEach(function(element) {";
        // Ensure the placeholder is set to the value, not the key
        $html .= "element.setAttribute('placeholder', '$safeValue');"; // Correctly set placeholder value
        $html .= "});";
    }
